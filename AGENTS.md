...
The Focus round setup keeps the session subtask outline visible beside the primary start action instead of hiding it under timer adjustments. The outline remains local to the active focus session and appears beside the running setup and focus timers. Tasks estimated at 30 minutes or longer also receive one optional suggested rhythm. Setup counts toward the task estimate, breaks are shown separately in the total clock time, and tasks of an hour or more are divided into focus rounds of no more than 30 minutes. Choosing the suggestion carries the user through each setup, focus, and break step, survives reloads in local session state, and still allows the task to be completed early or the timer controls to be customized. The active rhythm stays on one horizontally scrollable line at narrow widths so switching from a long suggested plan to a single focus duration does not move the timer controls.
...
Dragging or resizing one Google recurring occurrence now reconciles the full provider series in one serialized local transaction. This prevents each move from reinserting all occurrences beside the older local copies, and stale calendar rows now make the drag revert instead of leaving a client-only ghost. The checked-in Google event deduplication migration removes duplicate provider rows already produced by the old behavior while leaving CalDAV recurrence expansion untouched. Apply that migration before serving this version against an existing database.

Changing or deleting a recurring calendar event now requires an explicit scope choice. Drag, resize, the event editor, and quick-delete all ask whether the action applies only to the selected occurrence or to that point and the repeating schedule going forward; cancelling a drag or resize restores the event to its original time. Google single-occurrence edits patch the selected provider occurrence directly, avoiding a lookup from the newly chosen time that could modify the following occurrence instead.

Calendar mutations invalidate older visible-range requests so an event deleted during an overlapping load cannot reappear as a stale client row. A repeated delete of a row that the server already removed is treated as successful cleanup. Google sync and recurring-series reconciliation also discard cancelled or incomplete provider occurrences instead of rendering tombstones or failing a later move while rebuilding the series.

Provider event saves are authoritative independently of their follow-up work. Task auto-scheduling now runs quietly after calendar creation, movement, resizing, deletion, and sync, so a scheduling failure cannot claim that an already-saved calendar action failed or visually revert it. If Google accepts an update but the subsequent local recurring-series reconciliation fails, Sunnie keeps the selected local row aligned with the accepted change, returns success, and queues a full feed sync.

Google single-occurrence edits, including duration changes, persist the returned provider occurrence directly and close the editor without rebuilding the full recurring series. This avoids making a one-event edit wait on hundreds of occurrence inserts or a series reconciliation timeout. The event editor disables and relabels its submit action while a save is active so repeated clicks cannot start overlapping requests.
...
- Friends puts incoming requests first, presents connected people before the invite form on phones once someone is connected, and keeps an account-email invite visible for new accounts. Each accepted row separates “What they can see” from “What you can see,” explains the selected sharing level, and links to Calendar when shared time is available. Requests and visibility changes update immediately; failed writes restore the previous state with a visible error. Friend removal confirms that calendar sharing will stop, while sent requests can be canceled directly. Missing-account guidance points to the private planner host rather than assuming public signup is open.
