...
- `UserSettings.calendarStyle` persists independently as `classic` or `bujo`, so every colorway can use either calendar presentation. `src/lib/planner-themes.ts` layers semantic visual configuration over the stable color registry: reusable grid, border, typography, event, task, and all-day variants are exposed through presentation data attributes rather than theme-ID conditionals. The first Bujo version supports dot-grid, lined-paper, and graph-paper canvases; decorative handwritten headings; and highlight, washi, outline, and sticky-note item treatments. Sticker persistence, real theme assets, and ambient activation motion renderers are not implemented yet.
- Bujo paper patterns paint the visible FullCalendar body rather than sitting behind its internal layout layers. Winter outline events retain their palette color as a strong border and text treatment instead of looking like filled Classic cards, and sticky-note tasks include a visible folded corner.
...
- The five built-in themes now declare distinct stationery art direction in `planner-themes.ts`: Base uses soft garden paper and a dotted sidebar; Spring uses lined paper and a reusable scalloped accent; Summer uses gingham; Autumn uses kraft-paper cues plus the reusable plaid primitive; and Winter uses graph paper with outlined events. Each theme also reserves conceptual sticker, washi, and illustration pack IDs, three signature details, pattern intensity, decorative density, and one-shot motion. These declarations do not yet provide sticker placement/persistence or final artwork.
