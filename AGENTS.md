...
- `UserSettings.calendarStyle` persists independently as `classic` or `bujo`, so every colorway can use either calendar presentation. `src/lib/planner-themes.ts` layers semantic visual configuration over the stable color registry: reusable grid, border, typography, event, and task variants are exposed through presentation data attributes rather than theme-ID conditionals. The first Bujo version supports dot-grid, lined-paper, and graph-paper canvases; decorative handwritten headings; and marker, washi, outline, and sticky-note item treatments. Sticker persistence, theme assets, and ambient activation motion are not implemented yet.
