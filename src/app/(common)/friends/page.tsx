  ...
  return (
    <main className="min-h-full bg-[#fff9e8] p-4 text-[#3f432e] sm:p-5 lg:p-8">
      <div className="mx-auto max-w-5xl">
        ...
        {loading ? <div className="grid min-h-64 place-items-center"><Loader2 className="h-7 w-7 animate-spin text-[#84a75e]" /></div> : (
                ...
                {accepted.map((connection) => (
                  <article key={connection.id} className="rounded-xl border border-black/[0.06] bg-white p-4">
                    <div className="flex flex-wrap items-center gap-3">
                      <div className="grid h-10 w-10 place-items-center rounded-full bg-[#e6f0cf] font-semibold text-[#607044]">{(connection.friend.name || connection.friend.email || "F").slice(0, 1).toUpperCase()}</div>
            ...
            <aside className="space-y-5">
              <section className="rounded-2xl border border-black/[0.065] bg-white/75 p-5 shadow-sm">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  {incoming.map((connection) => <div key={connection.id} className="rounded-xl bg-[#eef3df] p-3"><p className="text-sm font-medium">{connection.friend.name || connection.friend.email}</p><p className="text-xs text-black/40">{connection.friend.email}</p><div className="mt-3 flex gap-2"><button onClick={() => void act(connection.id, "accept")} className="flex items-center gap-1 rounded-lg bg-[#64734a] px-3 py-1.5 text-xs text-white"><Check className="h-3.5 w-3.5" />Accept</button><button onClick={() => void act(connection.id, "decline")} className="rounded-lg px-3 py-1.5 text-xs text-black/50 hover:bg-black/5">Decline</button></div></div>)}
                </div>
              </section>
