...
function StepQuote({ stepIndex }: { stepIndex: number }) {
  return (
    <p className="mt-4 border-t border-[#e8dfbc] pt-3 text-[11px] italic leading-relaxed text-[#7a7356]">
      {formatIntentionQuote(ONBOARDING_STEPS[stepIndex].quote)}
    </p>
  );
}
...
function StepProgress({ stepIndex }: { stepIndex: number }) {
  return (
    <div aria-label={`Step ${stepIndex + 1} of ${ONBOARDING_STEPS.length}`}>
...
export function OnboardingTour() {
    ...
    void loadStatus()
      .then((nextStatus) => {
        ...
        if (nextStatus.onboardingVersion < CURRENT_ONBOARDING_VERSION) {
          ...
          setStepIndex(clampOnboardingStep(storedStep));
          setIsOpen(true);
        }
      })
      .catch((caught) => {
  ...
  }, [isOpen, pathname, router, stepIndex]);

  const moveTo = async (nextIndex: number) => {
    setError(null);
    if (ONBOARDING_STEPS[stepIndex].id === "connect") {
      try {
        await loadStatus();
      } catch {
    ...
    setStepIndex(clampOnboardingStep(nextIndex));
  };
  ...
  if (step.layout === "tour") {
    return (
      <section
        ...
        aria-live="polite"
        className="fixed inset-x-3 bottom-[calc(4.75rem+env(safe-area-inset-bottom))] z-[100] ml-auto max-w-sm rounded-3xl border border-[#ded6ad] bg-[#fffaf0]/[0.98] p-4 text-[#514d38] shadow-[0_20px_70px_rgba(61,56,34,0.24)] backdrop-blur sm:bottom-5 sm:right-5 sm:left-auto sm:p-5"
      >
