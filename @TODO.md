...
- [ ] **Page-by-page Sunnie UI rework** ([owner idea](FEATURE_IDEAS.md#ideas-were-fleshing-out)). Today, Upcoming, Tasks, Calendar, and Settings → Calendar accounts are in place. Tasks has direct capture, search, Focus, project choice and creation, a readable list, and distinct save versus scheduling feedback. Calendar has account-time-zone headings, a no-feed path to Connections, compact phone controls, rounded event blocks, visible Free events that do not reserve time, private naming, and Google/CalDAV popup/display reminder editing. Connections guides Google, Apple, and CalDAV setup, opens calendar selection after CalDAV connection, refreshes counts after adding a feed, and confirms account removal. Task sync stays optional. Signed-in local desktop and phone QA covered the Tasks journeys, new-account Calendar and Connections states, a simulated Apple connection and calendar add, private event naming, and failed and successful account removal. Next: verify reminder, private-name, and connection resync with live Google/CalDAV providers and review a dense imported calendar; then rework Friends.
