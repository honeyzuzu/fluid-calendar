...
- [ ] **Page-by-page Sunnie UI rework** ([owner idea](FEATURE_IDEAS.md#ideas-were-fleshing-out)). Today, Upcoming, Tasks, and the Calendar page pass are in place. Tasks has direct search, backlog guidance, Focus and details actions on cards, a readable list, All tasks as the entry view, starter General/Work/School/Hobbies projects, General assignment for unassigned and new tasks, in-form project creation, safe project deletion, and distinct save versus scheduling feedback. Calendar has account-time-zone headings, a clear no-calendar path to connections, compact phone controls, readable rounded event blocks, visible Free events that do not reserve time, safer feed controls, and private naming. Calendar event editing supports Google/CalDAV popup/display reminders. Signed-in local QA covered the Tasks journeys, a new account without a calendar, dense desktop and phone calendars, private names that leave the source title intact, feed removal, and saved Google reminder controls. Before closing Calendar, verify reminder and private-name resync with connected live Google/CalDAV accounts and review a dense imported calendar. Then choose the next page with the owner.
