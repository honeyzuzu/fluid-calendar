...
- [ ] **Page-by-page Sunnie UI rework** ([owner idea](FEATURE_IDEAS.md#ideas-were-fleshing-out)). Today, Upcoming, and the Tasks pass are in place. Tasks has direct search, backlog guidance, Focus and details actions on cards, a readable list, All tasks as the entry view, starter General/Work/School/Hobbies projects, General assignment for existing unassigned tasks and new captures, in-form project creation, safe project deletion, and distinct save versus scheduling feedback. Calendar now has rounded event blocks, shows missing imported titles as Busy or Free from provider availability, allows private Sunnie-only event names, and edits Google/CalDAV popup/display reminder schedules. Signed-in local QA passed new-account no-calendar capture to General, search, project quick-create, edit, Focus entry, completion, and supplemental tools at desktop and phone widths. Mobile Manage deleted a starter project, moved its task to General, and preserved the task. A forced scheduler failure after task creation left the task saved and displayed a separate warning. A simulated free/busy event accepted a private name without changing its source title. Next: a full Calendar page pass, including connected Google/CalDAV reminder and label resync and dense live-calendar visual review.
