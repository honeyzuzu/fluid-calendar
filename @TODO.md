...
The page-by-page Sunnie UI rework is complete across Today, Upcoming, Tasks, Calendar, Calendar accounts, Friends, Focus, Review, and the remaining Settings sections. The final Settings pass clarified calendar display versus scheduling, folded optional controls, added no-calendar guidance and failed-save rollback, repaired legacy reminder values, and verified task export/import. Signed-in local desktop and phone QA, unit tests, lint, type checking, and a production image build passed. See [AGENTS.md](AGENTS.md) for shipped behavior.

## Current work

- [ ] **Live calendar integration verification.** With connected Google and CalDAV test accounts, verify reminder and private-name resync, connection flows, and a dense imported calendar at desktop and phone widths. Local account and simulated-provider UI passes are complete; this needs real provider access.
