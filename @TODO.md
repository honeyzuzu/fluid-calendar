...
- [ ] **Page-by-page Sunnie UI rework** ([owner idea](FEATURE_IDEAS.md#ideas-were-fleshing-out)). First pass underway: Today makes the next task, completion, and capture immediate; Upcoming owns week-to-day planning; Tasks owns backlog capture and details; Calendar owns events and time; Focus opens directly from a selected task; regular users stay on Sunnie Base while admins retain theme tools. Complete when the Today-to-Focus-to-done journey works on mobile without setup, Tasks and Upcoming give captured work an obvious home, Calendar has no dead-end create action, navigation is consistent, and signed-in responsive QA confirms the flow. Then evaluate the deeper Plan and review screens, setup guidance, and remaining page density with owner feedback.
